...
def report(obs,mod,var,fun=['QM','DQM','QDM','UQM_reference','SDM'],y_init=0,y_wind=0):
    """
    This function generates two report of the performance of the different
    methods (QM, DQM, QDM, UQM_reference and SDM) available in the climQMBC package.
    ...
    Optional inputs:
        ...
        The methods supported are:
            ...
            d) 'UQM_reference': Unbiased Quantile Mapping
            Default: fun = ['QM','DQM','QDM','UQM_reference','SDM']
    ...
    Output:
        ...
        UQM_series: A column vector of monthly or annual modeled data 
                    (temperature or precipitation) corrected by the UQM_reference method.
                    If monthly frequency is specified, the length of this 
                    vector is 12 times the number of observed years 
                    [12 x y_mod, 1]. If annual frequency is specified, the 
                    length of this vector is equal to the number of observed 
    ...
    df_ind = ['Modeled','QM','DQM','QDM','UQM_reference','SDM']
    ...
    if 'UQM_reference' in fun:
        plt.plot(np.sort(UQM_series[y_obs*12:]),ecdf(UQM_series[y_obs*12:]),'c--',lw=1,label=r'UQM_reference$_f$')
    if 'SDM' in fun:
    ...
    if 'UQM_reference' in fun:
        plt.plot(UQM_series,'c',lw=1,label='UQM_reference')
    if 'SDM' in fun:
    ...
    if 'UQM_reference' in fun:
        plt.plot(mu_M[7],'c',lw=1,label='UQM_reference')
    if 'SDM' in fun:
    ...
    for w in range(len(y_wind)):
        ...
        if 'UQM_reference' in fun:
            plt.plot(mu_Mw[w][4],'c',lw=1,label='UQM_reference')
        if 'SDM' in fun:
    ...
    if 'UQM_reference' in fun:
        plt.plot(std_M[7],'c',lw=1,label='UQM_reference')
    if 'SDM' in fun:
    ...
    for w in range(len(y_wind)):
        ...
        if 'UQM_reference' in fun:
            plt.plot(std_Mw[w][4],'c',lw=1,label='UQM_reference')
        if 'SDM' in fun:
