...
#
#         t = pd.read_csv(path_clima + f'UQM/MPC_t2m_{m}_{x}_{bc_start}_2100_week_UQM.csv')
#         t = pd.read_csv(path_clima + f'UQM/MPC_pr_{m}_{x}_{bc_start}_2100_week_UQM.csv')
#
#         print(f'{m}_{x}:')
#         print(f'Columns: {t.shape[1]}')
#         print(f'Nans: {t.isna().any().sum()}')
    ...
    for x in experiments:
        ...
        for var in [0]:
            ...
            if var == 1:
                ...
                obs = obs_tm
                obs.loc[:, (obs.columns != 'week') & (obs.columns != 'year')] = obs.loc[:, (obs.columns != 'week') & (obs.columns != 'year')] + 273.15 # convert to deg. K for only positive values
                mod = mod_tm
                mod.loc[:, mod.columns != 'day'] = mod.loc[:, mod.columns != 'day'] + 273.15
                ...
                if ('year' not in catch) and ('week' not in catch):
                    if 'HUA' not in catch:
                        if catch in mod.keys():  # add this statement since mod has subset of catchments from obs
                            ...
                            obs_bc = obs[catch][
                                obs.year.isin([i for i in range(int(bc_start), int(bc_end) + 1)])].to_numpy()
            ...
            if var == 1:
                ...
                cor = cor - 273.15  # convert back to deg. C
                cor.fillna(axis=0, method='ffill', inplace=True)
                cor.insert(0, 'week', mod_tm.loc[bc_start:'2099'].index.isocalendar().week.tolist())
                cor.insert(0, 'year', mod_tm.loc[bc_start:'2099'].index.isocalendar().year.tolist())
                # cor['week'] = mod_tm.loc[bc_start:'2099'].index.isocalendar().week.tolist()
