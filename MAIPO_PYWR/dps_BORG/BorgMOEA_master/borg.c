...
void BORG_Copyright(FILE* fp) {
	...
	fprintf(fp, "If not, contact <info@borgmoea.org>.\n");
}
...
void BORG_Operator_SBX(BORG_Operator sbx, BORG_Solution* parents, BORG_Solution* offspring) {
	...
	int i;
	double y0, y1, betaq, beta, alpha, rand;
		...
		for (i=0; i<numberOfVariables; i++) {
			if (BORG_Random_uniform(0.0, 1.0) <= 0.5) {
				...
				if (dx > EPSILON) {
					...
					beta = 1.0 / (1.0 + (2.0 * (y0 - lb) / (y1 - y0)));
					alpha = 2.0 - pow(beta, distributionIndex + 1.0);
					rand = genrand_real2(); /* need random number between [0,1) */

					if (rand <= 1.0 / alpha) {
						alpha = alpha * rand;
						betaq = pow(alpha, 1.0 / (distributionIndex + 1.0));
					} else {
						alpha = alpha * rand;
						alpha = 1.0 / (2.0 - alpha);
						betaq = pow(alpha, 1.0 / (distributionIndex + 1.0));
					...
					x0 = 0.5 * ((y0 + y1) - betaq * (y1 - y0));
			
					/* generate second offspring */
					beta = 1.0 / (1.0 + (2.0 * (ub - y1) / (y1 - y0)));
					alpha = 2.0 - pow(beta, distributionIndex + 1.0);
					...
					offspring[0]->variables[i] = x0;
					offspring[1]->variables[i] = x1;
				}
			}
		}
	}
...
void BORG_Algorithm_inject(BORG_Algorithm algorithm, BORG_Solution solution, int evaluate, int incrementNFE) {
	BORG_Validate_pointer(algorithm);
	BORG_Validate_pointer(solution);
	...
	BORG_Population_add(algorithm->population, solution);
	BORG_Archive_add(algorithm->archive, solution);	
