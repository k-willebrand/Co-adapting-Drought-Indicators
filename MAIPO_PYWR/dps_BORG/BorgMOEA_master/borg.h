...
/**
 * Inject a solution into the algorithm.  The solution is added to the
 * population and archive.  If evaluate is true, then the solution is first
 * evaluated.  If incrementNFE is true, then the NFE counter is incremented.
 * Note that solutions injected in this manner may not be added to the
 * population or archive if they are dominated by existing members of the
 * population or archive.
 *
 * When used to initialize the population, if incrementNFE is true, then Borg
 * will not perform any initialization.  Instead, if incrementNFE is false, then
 * Borg will perform its regular initialization, possibly replacing any
 * dominated solutions.
 */
BORG_API void BORG_Algorithm_inject(BORG_Algorithm algorithm, BORG_Solution solution, int evaluate, int incrementNFE);
